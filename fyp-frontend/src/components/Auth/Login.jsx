...
const Login = () => {
  ...
  const urlData = location.state?.url || null;
  const resultData = location.state?.data || null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (from === '/result' && urlData && resultData) {
          navigate('/result', { state: { url: urlData, data: resultData } });
        } else {
          navigate(from);
        }
      } else {
      ...
      >
        <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-teal-100/50">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-600 mt-1">Sign in to your LinkGuard account</p>
          </div>
