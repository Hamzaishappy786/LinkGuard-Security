// src/components/MultipleUrlResults.jsx
import { useState } from 'react';
import { motion } from 'framer-motion';
import { useNavigate } from 'react-router-dom';
import { downloadMultipleReport } from '../utils/api';
import { useAuth } from '../context/AuthContext';

export default function MultipleUrlResults({ results }) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="content-container mb-6">
        ...
        >
          <h2 className="text-xl font-bold text-gradient mb-4">Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-green-50 rounded-xl">
              <div className="text-2xl font-bold text-green-600">{safeCount}</div>
              <div className="text-sm text-slate-600">Safe URLs</div>
            </div>
            <div className="text-center p-4 bg-red-50 rounded-xl">
              <div className="text-2xl font-bold text-red-600">{phishingCount}</div>
              <div className="text-sm text-slate-600">Phishing URLs</div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-bold ${
                      ...
                      {result.label === 'Safe' ? '✅ Safe' : result.label === 'Phishing' ? '❌ Phishing' : '⚠️ Error'}
                    </div>
                    <div className="text-xs text-slate-500 mt-1 break-all max-w-md">{result.url}</div>
                  </div>
                </div>
              </div>
            </motion.div>
