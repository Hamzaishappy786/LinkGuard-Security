// src/pages/MultipleResults.jsx
import { useLocation, useNavigate } from 'react-router-dom'
import { motion, AnimatePresence } from 'framer-motion'
import { useState } from 'react'
import FeatureTable from '../components/FeatureTable.jsx'
import { checkDownloadAuth, downloadMultipleReport } from '../utils/api.js'
...
export default function MultipleResults(){
    ...
    return (
      <div className="content-container">
        <div className="card p-8 text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-900 mb-2">No Results to Show</h3>
          <p className="text-slate-600 mb-6">Please analyze links first.</p>
  ...
  const safeCount = results.filter(r => r.label === 'Safe').length
  const phishingCount = results.filter(r => r.label === 'Phishing').length
  ...
  const handleDownload = async (format) => {
    ...
    try {
      const result = await downloadMultipleReport(results, format);
  ...
  const handleLoginRedirect = () => {
    navigate('/login', { state: { from: '/multiple-results', results } });
  };
    ...
    <div className="min-h-screen">
      ...
      <section className="content-container pt-8 pb-6">
        <div className="text-center">
          <h1 className="text-3xl md:text-4xl font-bold text-gradient mb-2">Multiple URLs Analysis Results</h1>
          <p className="text-slate-600">Detailed security assessment of the checked URLs</p>
        </div>
      </section>
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-xl">
              <div className="text-2xl font-bold text-green-600 dark:text-green-400">{safeCount}</div>
              <div className="text-sm text-slate-600 dark:text-slate-400">Safe URLs</div>
            </div>
            <div className="text-center p-4 bg-red-50 dark:bg-red-900/20 rounded-xl">
              <div className="text-2xl font-bold text-red-600 dark:text-red-400">{phishingCount}</div>
              <div className="text-sm text-slate-600 dark:text-slate-400">Phishing URLs</div>
            </div>
            <div className="text-center p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl">
              <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">{errorCount}</div>
        ...
        <div className="card p-6">
          <div className="flex flex-wrap gap-2 mb-6">
            {results.map((result, index) => (
              ...
              >
                {result.label === 'Safe' ? '✅' : result.label === 'Phishing' ? '❌' : '⚠️'} 
                {result.url.substring(0, 20)}{result.url.length > 20 ? '...' : ''}
              </button>
            ))}
          </div>
          ...
          <motion.div
            ...
            className={`border-2 rounded-xl p-6 ${
              activeResult.label === 'Safe'
                ? 'border-green-200 bg-gradient-to-br from-green-50 to-emerald-50'
                : activeResult.label === 'Phishing'
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-lg font-bold ${
                    ...
                    {activeResult.label === 'Safe' ? '✅ Safe URL' : 
                     activeResult.label === 'Phishing' ? '❌ Phishing URL' : 
                     '⚠️ Error'}
                  </div>
                  <div className="text-xs text-slate-500 mt-1 break-all max-w-md">{activeResult.url}</div>
                </div>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="card p-4 bg-white/70 dark:bg-slate-800/70">
                      ...
                      <div className="space-y-3">
                        ...
                        <button 
                          onClick={() => navigator.clipboard.writeText(activeResult.url)}
                          className="flex items-center gap-2 text-teal-600 hover:text-teal-700 font-medium"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            ...
            >
              <div className="text-center mb-4">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">Login Required</h3>
